...
from common.files import buffer_config_dir, CONFIG_FILE_EXTENSION
...
def remove_config_comments():
    configs = [
        y
        for x in os.walk(processed_dir)
        for y in glob(os.path.join(x[0], f"*{CONFIG_FILE_EXTENSION}"))
    ]
...
def compress_configs():
    ...
    file = f"{buffer_config_dir}/{name}.7z"
